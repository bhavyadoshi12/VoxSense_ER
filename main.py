...
def home_page():
    ...
    with col2:
        ...
        if not st.session_state.torch_available: 
            st.warning("⚠️ PyTorch not available - Using enhanced rule-based analysis")
        else: 
        ...
        if st.button("🚀 Begin Emotional Analysis →", use_container_width=True, key="start_analysis"):
...
def user_details_page():
    ...
    with st.form("user_details_form"):
        ...
        submitted = st.form_submit_button("🎤 Continue to Voice Analysis →", use_container_width=True)
...
def live_recording_section():
    ...
    with col2:
        ...
        if not is_recording:
            if st.button("🎤 Start Recording Session", use_container_width=True, key="start_rec"):
                reset_analysis() 
            ...
            # Stop Button
            if st.button("⏹️ Stop & Analyze", key="stop_rec", use_container_width=True):
                st.session_state.recording = False 
...
def audio_upload_section():
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Analyze Audio File", key="process_upload", use_container_width=True):
            reset_analysis() 
            temp_file_path = None
            try:
...
def results_page():
    ...
    if st.button("📄 Download Premium Emotional Intelligence Report", use_container_width=True, type="primary"):
        generate_pdf_report()
